...
6. [Complete Tech Stack](#6-complete-tech-stack-what-we-use-to-do-what)
...
* 🔒 **Proximity-Based Trust & Real-World Accountability:** College-email verification (`.edu` / college domains) anchors every account to a real campus identity, eliminating anonymous ghosting, fake accounts, and low-trust interactions.
* ⚡ **Zero Cold-Start Friction via GitHub Forensics:** Instead of asking students to fill out long, untrustworthy self-assessment forms, KEVIN automatically analyzes their GitHub coding activity to construct an objective, empirically verified skill profile upon onboarding.
* 🪙 **Incentive-Driven Peer Economy:** A structured credit economy rewards students who spend time teaching peers and enables them to spend earned credits to learn skills from senior or specialized developers, replacing goodwill with a sustainable campus flywheel.
...
Most existing hackathon team-finders rely on naive keyword matching: _Candidate writes "Machine Learning" on profile ⟹ Platform marks them as ML Expert._ This leads to mismatched teams, missing critical skills, and team breakdown during 36-hour hackathons.
...
* **The Vulnerability:** A student forks a complex open-source Machine Learning or Blockchain repository. Naive tools scan the language and topics, see "Python" and "PyTorch", and award the student a 95% ML score—even if the student contributed 0 lines of code.

✅ **Our Solution (KEVIN's Approach):**

* Detects `isFork` via GitHub REST APIs and traces the upstream parent repository.
* Queries the GitHub Contributors and Commits endpoints for the candidate's username.
* Calculates the **Personal Contribution Ratio**:
  $$
  \text{Contribution \%} = \left( \frac{\text{Commits}_{\text{user}}}{\text{Commits}_{\text{total}}} \right) \times 100
  $$
* **Result:** Penalizes forked repositories where personal contribution is low (<20%) and entirely excludes upstream commits from the candidate's scoring.

### ⚠️ Loophole 2: The Cosmetic / "README" Commit Loophole (Contribution Depth)

* **The Vulnerability:** A candidate makes 30 commits to an ML repository. Traditional tools see 30 commits and conclude high activity. In reality, all 30 commits were typo fixes in `README.md` or `.gitignore` modifications. The candidate never wrote functional code.

✅ **Our Solution (KEVIN's Approach):**

* **File Syntax AST Classification:** Inspects individual files modified across user commits and partitions them:
...
* Calculates the **Code-to-Document Density Ratio**:
  $$
  \text{CodeRatio} = \left( \frac{\text{Files}_{\text{code}}}{\text{Files}_{\text{total}}} \right) \times 100 \quad | \quad \text{DocRatio} = \left( \frac{\text{Files}_{\text{doc}}}{\text{Files}_{\text{total}}} \right) \times 100
  $$
* **Result:** Automatically flags candidates whose modifications are ≥75% documentation and ≤15% code as *Cosmetic Contributors*, heavily penalizing their technical skill confidence.
* **Semantic Grounding via Gemini 3.5 Flash:** Rather than asking an LLM "Did this person write this code?", Gemini is grounded strictly in the candidate's touched modules (`model_training.py` vs `README.md`) to verify actual engineering capability.
...
* **S_contrib (30%)**: Scaled personal contribution ratio $C_{\text{user}} / C_{\text{total}}$ (100 for sole author, ≤15 for <15% contribution).
* **S_orig (20%)**: Originality index (100 for original repos, 15−70 for forks based on independent work).
* **S_depth (20%)**: Code depth (100 for CodeRatio ≥70, penalized to 10 for cosmetic-only edits).
* **S_time (15%)**: Iterative consistency (100 for ≥3 months iterative development; 15 for single-commit dumps).
* **S_relevance (15%)**: Alignment of repository tech stack with project requirements.

**2. Three-Tier Evidence Badges**

* 🟢 **Strong Evidence (≥70):** Original repo, substantial personal code, active over months.
...
$$
\text{Calibrated Score} = \begin{cases} 
10.0 / 10, & \text{Strong Evidence (Verified Code Contributor)} \\ 
7.5 / 10, & \text{Moderate Evidence} \\ 
3.5 / 10, & \text{Weak Evidence (Fork / README Loophole)} 
\end{cases}
$$
...
| **AI & Generative Reasoning** | Google Gemini 3.5 Flash    | 1. Hackathon theme parsing into technical roles.2. Grounded code-level skill extraction from modified modules and commit diffs.    |
| **GitHub Forensics API**      | GitHub REST API v3 / Axios | Fetching repository trees, fork parents, author commits, commit file diffs, and contributor statistics.                            |
| **Authentication & Trust**    | JWT & Bcrypt.js            | Secure student session management and college ID unique indexing.                                                                  |
| **Real-Time Comm.**           | Socket.io                  | Live team invites, notifications, and real-time match updates.                                                                     |
...
*(Refer to the Multi-Dimensional Evidence Engine diagram in Section 5 for the core algorithmic data flow)*
...
<div align="center">
  <p><strong>👥 Team Innovate365</strong></p>
  <p>Thanvi — AI & Backend Architecture<br>
  Rajath — Database & GitHub Integration<br>
  Gahan — Core Backend & Matching Logic</p>
  <p><em>Built with ❤️ for DSU DEVHACK 3.0 at Sahyadri College of Engineering & Management.</em></p>
</div>